...
def merge_bins(target, bins, *counts, **kwargs):
    ...
    return new_counts, np.array(new_bins)
...
class Grim_Brunelle_merger(object):
    def __init__(self, bins, *counts) -> None:
        ...
        *self.merged_counts, self.post_stats_merge_bins = merge_bins(0.05*np.mean( stats_for_mean ), 
        ...
        self.non_local_indices = dict(zip(starting_indices, starting_indices)) #to keep track of counts merging non locally
    ...
    def __grim_metric__(self, i, j):
        ...
        terms = hBar/h
        numerator = (np.prod(terms)**(1./self.n))[-1]
        denomenator = np.sum(terms)
        
        metric = (numerator/denomenator)#*np.exp(np.sum(h + hBar)/np.sum(overall_counts) - 1 )
        return metric
    
    def local_merge(self, i, j):
        ...
        if i == j + 1: #merges count i with the count behind of it
            if i > self.n_items - 1 or i <= 1:
                raise ValueError("YOU IDIOT WHY WOULD YOU MERGE ON THE EDGES THAT LOSES THE RANGE")
            temp_counts = np.concatenate( (self.counts_to_merge[:j], [self.counts_to_merge[j] + self.counts_to_merge[i]], self.counts_to_merge[i+1:]) )
            temp_edges = np.concatenate( (self.local_edges[:i], self.local_edges[i+1:]) )
        elif i == j - 1: #merges count i with the count in front of it
            if i > self.n_items - 1 or i < 1:
                raise ValueError("YOU IDIOT WHY WOULD YOU MERGE ON THE EDGES THAT LOSES THE RANGE")    
            temp_counts = np.concatenate( (self.counts_to_merge[:i], [self.counts_to_merge[i] + self.counts_to_merge[j]], self.counts_to_merge[j+1:]) )
    ...
    def run_local(self, target_bin_number):
        while len(self.counts_to_merge) > target_bin_number:
            combinations = {}
            scores = {}
            
            temp_counts_ahead = np.zeros(shape=(self.counts_to_merge.shape[0], self.counts_to_merge.shape[1] - 1), dtype=float)
            temp_counts_behind = np.zeros(shape=(self.counts_to_merge.shape[0], self.counts_to_merge.shape[1] - 1), dtype=float)
            
            for i in range(1, self.n_items - 1): #don't merge edge bins/counts!
                for j in self.n:
                    temp_counts_ahead[j], temp_bins = self.local_merge(i, i+1) #merge_ahead(self.counts_to_merge[j], new_bins, i)
                score_ahead = self.__grim_metric__(i, i+1)
                combinations[i] = (temp_counts_ahead, temp_bins)
                scores[i] = score_ahead
            
            for i in range(1, self.n_items - 1): #don't merge edge bins/counts!
                for j in self.n:
                    temp_counts_behind[j], temp_bins = self.local_merge(i, i-1)#merge_behind(self.counts_to_merge[j], new_bins, i)
                score_ahead = self.__grim_metric__(i, i+1)
                combinations[i] = (temp_counts_ahead, temp_bins)
                scores[i] = score_ahead
            
            # print(scores)
            self.counts_to_merge, self.local_edges = combinations[ max(scores, key=scores.get) ]
